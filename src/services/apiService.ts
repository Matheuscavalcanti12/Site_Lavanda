...
export const apiService = {
  async addToCart(pedidoId: number, produtoId: number, quantidade: number) {
  ...
  async getCarrinho(pedidoId: number) {
    const response = await fetch(`${API_BASE_URL}/pedido/${pedidoId}/itens`);
  ...
  async removeFromCart(pedidoId: number, produtoId: number) {
    const response = await fetch(`${API_BASE_URL}/pedido/item`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
